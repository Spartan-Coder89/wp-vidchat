document.addEventListener('alpine:init', () => {
  ...
  Alpine.data('vidchat', () => ({
    ...
    async input_files_send_files(event) {
      ...
      if (files.length > 0) {
        ...
        if (Object.keys(this.media_connection_collection).length > 0) {
          ...
          this.my_stream = await navigator.mediaDevices.getUserMedia(this.media_devices_constraints)
          document.getElementById('local_stream').srcObject = this.my_stream
          const my_stream_video_tracks = this.my_stream.getVideoTracks()[0]
          const my_stream_audio_tracks = this.my_stream.getAudioTracks()[0]
          
          for (const key in this.media_connection_collection) {
            let peer = this.media_connection_collection[key].peerConnection.getSenders().find(s => s.track.kind === my_stream_video_tracks.kind) //  This here is a track finder
            peer.replaceTrack(my_stream_video_tracks)
          }

          for (const key in this.media_connection_collection) {
            let peer = this.media_connection_collection[key].peerConnection.getSenders().find(s => s.track.kind === my_stream_audio_tracks.kind) //  This here is a track finder
            peer.replaceTrack(my_stream_audio_tracks)
          }
        }
      }
